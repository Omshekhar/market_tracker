...
if os.path.exists(FILE_NAME):
    ...
    existing_df = pd.DataFrame(columns=['timestamp', 'symbol', 'price', 'volume','prev_price'])

def gett_prev_price(symbol):
    # Filter the Old data for this symbol and get the very last entry
    symbol_data = existing_df[existing_df['symbol'] == symbol]
    if not symbol_data.empty:
        return symbol_data['price'].iloc[-1]
    return 0
records = []
for s in SYMBOLS:
    res = fetch_data(s)

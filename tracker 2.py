...
def get_prev_price(symbol):
    # Filter the old data for these symbols and get very latest entry
    last_entry = df_old[df_old['symbol'] == symbol]
    if not last_entry.empty:
        return last_entry['price'].iloc[-1]
    return 0

# 2. When you fetch new data.
records = []
for s in SYMBOLS:
    res = fetch_data(s)
    if res and '05. price' in res:
        ...
        # --- GET THE PREVIOUS PRICE HERE ---
        prev_price = get_prev_price(s)

# 2. Logic Gate: Check the last recorded entry for THIS symbol
        last_entry = df_old[df_old['symbol'] == symbol].tail(1)
        if not last_entry.empty:
